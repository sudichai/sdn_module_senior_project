...
"""
Wireless mesh network simulation with DDoS ICMP traffic generation.
"""
...
def generate_ddos_icmp(sender, receiver_ip, duration):
    """
    Generate DDoS ICMP traffic with specified packet counts.
    ...
    info(f"*** Generating DDoS ICMP traffic from {sender.name} to {receiver_ip}\n")
    ...
    while time.time() < total_end_time:
        ...
        info(f"[{datetime.now()}] {sender.IP()} -> {receiver_ip}: Start DDoS ICMP (size={packet_size} bytes, rate={rate_desc[rate]}, count={packet_count}, duration={burst_duration:.2f}s)\n")
...
def create_topology():
    ...
    time.sleep(60)
    info("*** Starting DDoS ICMP traffic\n")
    generate_ddos_icmp(attacker1, server1.IP(), duration=7200)
