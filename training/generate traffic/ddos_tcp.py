...
"""
Wireless mesh network simulation with DDoS TCP traffic generation.
"""
...
def generate_ddos_tcp(sender, receiver_ip, duration):
    """
    Generate DDoS TCP traffic with specified packet counts.
    
    Args:
        sender: The station sending the TCP traffic.
    ...
    rate_map = {1000: "u1000", 10000: "u100", 100000: "u10"}
    rate_desc = {1000: "medium (1000 pps)", 10000: "high (10000 pps)", 100000: "very high (100000 pps)"}
    
    start_time = time.time()
    total_end_time = start_time + duration
    info(f"*** Generating DDoS TCP traffic from {sender.name} to {receiver_ip}\n")
    ...
    while time.time() < total_end_time:
        ...
        burst_duration = random.uniform(3, 15)  # Shorter bursts for DDoS
        packet_count = int(rate * burst_duration)
        burst_start_time = time.time()

        info(f"[{datetime.now()}] {sender.IP()} -> {receiver_ip}: Start DDoS TCP (size={packet_size} bytes, rate={rate_desc[rate]}, count={packet_count}, duration={burst_duration:.2f}s)\n")
        
        sender.cmd(f"hping3 --syn -d {packet_size} --interval {density} -c {packet_count} {receiver_ip} -q &")
        
        expected_duration = packet_count / rate
        time.sleep(expected_duration + 0.5)  # Reduced buffer for faster cycling
...
def create_topology():
    ...
    time.sleep(60)
    info("*** Starting DDoS TCP traffic\n")
    generate_ddos_tcp(attacker1, server1.IP(), duration=7200)
