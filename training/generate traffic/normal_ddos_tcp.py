...
"""
Wireless mesh network simulation with TCP traffic generation.
"""
...
def generate_tcp(sender, receiver_ip, duration, attack_type='normal', burst_duration=10):
    """
    Generate TCP traffic with specified criteria (normal or DDoS).
    Args:
    ...
    info(f"*** Generating {attack_type.upper()} TCP traffic from {sender.name} to {receiver_ip}\n")
    ...
    while time.time() < total_end_time:
        ...
        info(f"[{datetime.now()}] {sender.IP()} -> {receiver_ip}: Start TCP (size={packet_size} bytes, rate={rate_desc[rate]}, count={packet_count}, duration={burst_duration:.2f}s)\n")
...
def create_topology():
    ...
    import threading
    duration = 7200  # Total duration for both attacks
    ...
    while time.time() < total_end_time:
        ...
        t1 = threading.Thread(target=generate_tcp, args=(attacker1, server1.IP(), duration, 'normal', burst_duration))
        t2 = threading.Thread(target=generate_tcp, args=(attacker2, server1.IP(), duration, 'ddos', burst_duration))
