...
def generate_normal_tcp(sender, receiver_ip, duration):
    """
    Generate TCP traffic with calculated packet counts.
    ...
    info(f"*** Generating TCP traffic from {sender.name} to {receiver_ip}\n")
...
def create_topology():
    ...
    generate_normal_tcp(attacker1, server1.IP(), duration=7200)
